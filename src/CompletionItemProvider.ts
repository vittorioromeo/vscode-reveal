...
export default (configDesc: ConfigurationDescription[]) => {
  ...
  ['markdown', 'majsdown'].forEach(language => {
    for (const e of enumValueProviders) {
      disposables.push(languages.registerCompletionItemProvider(language, e))
    }

    const mainProvider = languages.registerCompletionItemProvider(language, createCompletionItemsProvider(completionItems))
